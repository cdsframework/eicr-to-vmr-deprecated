...
-->
    ...
    <xsl:template name="check-external-image-whitelist">
        ...
        <xsl:choose>
            <xsl:when test="string-length($current-whitelist) &gt; 0">
                ...
                <xsl:choose>
                    <xsl:when test="starts-with($image-uri, $whitelist-item)">
                        ...
                        <xsl:message><xsl:value-of select="$image-uri"/> is in the
                            whitelist</xsl:message>
                    </xsl:when>
                    <xsl:otherwise>
            ...
            <xsl:otherwise>
                <p>WARNING: non-local image found <xsl:value-of select="$image-uri"/>. Removing. If
                    you wish non-local images preserved please set the limit-external-images param
                    to 'no'.</p>
                <xsl:message>WARNING: non-local image found <xsl:value-of select="$image-uri"/>.
                    Removing. If you wish non-local images preserved please set the
                    limit-external-images param to 'no'.</xsl:message>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
                ...
                <xsl:if
                    ...
                    <xsl:choose>
                        <xsl:when
                            ...
                            <xsl:call-template name="check-external-image-whitelist">
                                <xsl:with-param name="current-whitelist"
                                    select="$external-image-whitelist"/>
                                <xsl:with-param name="image-uri" select="$image-uri"/>
                            </xsl:call-template>
                            <!--
    ...
    <xsl:template name="show-actClassCode">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$clsCode = 'CASE'">
                <xsl:text>trigger code display term</xsl:text>
            </xsl:when>
